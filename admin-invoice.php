...
if($booking_status == 2 or $booking_status==1){
    ...
    $FlightNumber=$datas->CommitBooking->BookingDetails->JourneyList->FlightDetails->Details[0][0]->FlightNumber;
    $CabinClass=$datas->CommitBooking->BookingDetails->JourneyList->FlightDetails->Details[0][0]->CabinClass;
    $GDSPNR= $datas->CommitBooking->BookingDetails->PNR;
    $airlinespnr = $datas->CommitBooking->BookingDetails->JourneyList->FlightDetails->Details[0][0]->AirlinePNR;
    $Passengers=$datas->CommitBooking->BookingDetails->PassengerDetails;
    $Price=$datas->CommitBooking->BookingDetails->Price;
    $journey=$datas->CommitBooking->BookingDetails->JourneyList->FlightDetails->Details[0];
    $BasicFare=$Price->PriceBreakup->BasicFare;
    $Tax=$Price->PriceBreakup->Tax;
    $ticket_price=$BasicFare+$Tax;
    $comm=intval($amount-$ticket_price);
    foreach($Passengers as $passenger){
        $psgr.='<tr>
                        <td class="text-center">'.$passenger->PassengerId.'</td>
                        <td class="text-center text-capitalize">'.$passenger->FirstName.' '.$passenger->LastName.'</td>
                        <td class="text-center">'.$passenger->TicketNumber.'</td>
                    </tr>';
    }


    $row=mysqli_fetch_array(mysqli_query($con,"SELECT image from airlines where name like '%$airline%' "));
    if($row)
        $image=$row['image'];
    if(empty($image))
        $image="https://darwintrip.com/img/flight-default.png";
    foreach($journey as $airdetails){
        $date_o = new DateTime($airdetails->Origin->DateTime);
        $ddate_o=  date("D, M j, g:i a", strtotime($airdetails->Origin->DateTime));
        $date_d= new DateTime($airdetails->Destination->DateTime);
        $ddate_d=  date("D, M j, g:i a", strtotime($airdetails->Destination->DateTime));
        $airD.='<tr><td class="text-center"><b><img class="airline-logo" alt="'.$FlightNumber.'" src="'.$image.'" width="100px;" title="Image Title"><br>'.$airline.'</b> <br> '.$FlightNumber.'</td>
                        <td class="text-center"><b>'.$airdetails->Origin->CityName.'</b> <br> '.$ddate_o.'<br>Terminal: '.$airdetails->Origin->Terminal.'</td>
                        <td class="text-center"><b>'.$airdetails->Destination->CityName.'</b> <br> '.$ddate_d.'<br>Terminal: '.$airdetails->Destination->Terminal.'</td><tr>';
    }
...
/* =================================== */
        body {
            ...
            font-size: 14px;
            line-height: 22px;
        }.table  thead{	background-color:#eee;}.card-header{	    background-image: linear-gradient(96deg, #53b2fe, #065af3);		color:#fff;}
...
/* =================================== */
    ...
    <style>	     .table td, .table th {           padding: 6px;        }		.card-body {    -ms-flex: 1 1 auto;    flex: 1 1 auto;    padding: 7px;}.table {    margin-bottom: 0rem;    }
    .mt_20{
        margin-top: 20px;
    }
        .fn_22{
...
<div class="main">
    <div class="container-fluid invoice-container print-container" id="content">
        <div class="row align-items-center">
            <div class="col-sm-7 text-center text-sm-left"><img src="https://www.darwintrip.com/img/darwintriplogo.png" title="Darwintrip"/></div>
            <div class="col-sm-5 text-center text-sm-right">
                <p class="mb-0">Booking ID - <?php echo $appReference; ?></p>
                <p class="mb-0">Booked on - <?php echo date('F j, Y');  ?></p>
            </div>
        </div>
        <hr>
        <div class="row">
            ...
            <div style=" margin-top:15px;   text-align: right;" class="col-sm-6">
                ...
                <table style="border: 2px solid #000;" class="table table-bordered">
                    ...
                    <tr>
                        <td class="text-center">
                            <b>AIRLINES PNR</b> <br>
                            <h4><?php echo $airlinespnr ?></h4>
                        </td>
                        <td class="text-center"> 
                            <b>GDS PNR</b> <br>
                            <h4><?php echo $GDSPNR ?></h4>
                        </td>
                    </tr>
        ...
        <br />
        <div class="card">
            <div class="card-header"> <span class="font-weight-600 text-4">Flight Details</span> </div>
            <div class="card-body">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-bordered">
                        <thead>
                        <tr>
                            <td class="text-center"><strong>Passenger Id</strong></td>
            ...
            <div class="col-sm-12">
                <div class="table-responsive">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                        <tr>
                            <td><span class="text-3"><span class="font-weight-500">Air Fare</span><br>                           </td>
                            <td class="text-center"><?php echo $BasicFare ?></td>
                        </tr>
                        <tr>
                            <td><span class="text-3"><span class="font-weight-500">Taxes & Fees</span><br>                           </td>
                            <td class="text-center"><?php echo $Tax ?></td>
                        </tr>
                        <tr>
                            <td><span class="text-3"><span class="font-weight-500">Convinience fees(+) </span>                          </td>
                            <td class="text-center"><?php echo $comm ?></td>
                        </tr>

                        <tr>
                            <td><span class="text-3"><span class="font-weight-500"><strong>Grand Total</strong></span><br>                           </td>
        ...
        <br>
        <h6 class="fn_22" style="color:#000;"><strong>Important Information</strong></h6>
        <ul class="text-1">
